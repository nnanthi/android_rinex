...
Original Version by rokubun for converting into RINEX 2 (GPS) 
Entended to RINEX 3 (GPS, GLONASS, BDS, Galileo, and QZSS) by nnanthi  
"""
import argparse
import datetime
import math
...
GLONASS_L1_FD = 0.5625e6
BDS_B1_FREQ = 152.6 * 10.23e6
GPS_L1_WAVELENGTH = SPEED_OF_LIGHT / GPS_L1_FREQ
BDS_B1_WAVELENGTH = SPEED_OF_LIGHT / BDS_B1_FREQ
GPS_WEEKSECS = 604800 # Number of seconds in a week
GPS_DAYSECS = 86400 # Number of seconds in a day
BDS_GPS_TIME_DIFF = 14.0 #
GAL_GPS_TIME_DIFF = 13.0 #  Need testing
MOS_UTC_TIME_DIFF = -3*3600.0
NS_TO_S = 1.0e-9
...
def rinex_header(runby=None, marker=None, observer=None, agency=None,
                 ...
                 approx_position=[0,0,0], antenna_hen=[0.0, 0.0, 0.0], rinex3=False):
    """
    ...
    h += "{0:<60}COMMENT\n".format("Extended for RINEX 3 by Nanthi")
    h += "{0:<60}COMMENT\n".format("Reach @ nanthi@gmail.com for any question on this extension")
    ...
    # Observables
    if rinex3:
        h += "G    4 C1C S1C L1C D1C                                      SYS / # / OBS TYPES\n"
        h += "R    4 C1C S1C L1C D1C                                      SYS / # / OBS TYPES\n"
        h += "C    4 C2I S2I L2I D2I                                      SYS / # / OBS TYPES\n"
        h += "E    4 C1C S1C L1C D1C                                      SYS / # / OBS TYPES\n"
        h += "J    4 C1C S1C L1C D1C                                      SYS / # / OBS TYPES\n"
    else:
...
def get_glonass_slots(slotfile):
    slotdict={}
    ...
    for line in sfh:
        if line.startswith("R"):
            ...
            gl-=1
            if gl == 0:
                slotdict[prn] = int(float(line[61:80].replace("D", "E")))
...
class RinexBatch:
    ...
    def print(self,rinex3):
        ...
        # Check for repeated entries. In this case skip
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rinex3', dest='rinex3', action='store_true', default=False,
                        help="Output RINEX 3 format otherwise RINEX 3")
    parser.add_argument('--slotfile', metavar='<input navigation file>', type=str, default=None,
                        help="GLONASS navigation file to get slot numbers:"+
                            "A work around for not having slot number or frequency for GLONASS."+
                            "If not set (default), Output RINEX will not include GLONASS data")
    parser.add_argument('--leapsec', dest='leapsec', type=int, default=18,
                        help="LEAP seconds to determine GLONASS time"+
                            "If not set (default) which is current value (as of 2017)")
    ...
    slotdict = get_glonass_slots(args.slotfile)
    ...
    for line in fh:
        ...
        gpsdow = math.floor((-fullbiasnanos * NS_TO_S - gpsweek * GPS_WEEKSECS) / GPS_DAYSECS)
        ...
        if rinex_batch is None:
            ...
            out.write(rinex_batch.print(args.rinex3))
            rinex_batch = RinexBatch(epoch)
        ...
        if values['ConstellationType'] == CONSTELLATION_BEIDOU and not args.rinex3:
            continue
        ...
        if values['ConstellationType'] == CONSTELLATION_BEIDOU:
            tTxSeconds += BDS_GPS_TIME_DIFF;
        elif values['ConstellationType'] == CONSTELLATION_GALILEO:
            tTxSeconds += GAL_GPS_TIME_DIFF;
        elif values['ConstellationType'] == CONSTELLATION_GLONASS:
            tTxSeconds += (gpsdow*GPS_DAYSECS+MOS_UTC_TIME_DIFF +args.leapsec);
        ...
        # specification)
        if values['ConstellationType'] == CONSTELLATION_BEIDOU:
            l1 = values['AccumulatedDeltaRangeMeters'] / BDS_B1_WAVELENGTH
            d1 = - values['PseudorangeRateMetersPerSecond'] / BDS_B1_WAVELENGTH
        elif values['ConstellationType'] == CONSTELLATION_GLONASS:
            wavelength = SPEED_OF_LIGHT/(GLONASS_L1_FREQ+sln*GLONASS_L1_FD)
            l1 = values['AccumulatedDeltaRangeMeters'] / wavelength
            d1 = - values['PseudorangeRateMetersPerSecond'] / wavelength
        else:
